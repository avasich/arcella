...
async fn get_current_mtimes(
    paths: &[PathBuf],
) -> ArcellaResult<HashMap<PathBuf, std::time::SystemTime>> {
...
async fn ensure_main_config_exists(
    config_dir: &Path,
) -> ArcellaResult<(PathBuf, Vec<ConfigLoadWarning>)> {
    ...
    fs::create_dir_all(config_dir).await.map_err(|e| ArcellaError::IoWithPath {
        source: e,
    ...
    if !template_path.exists() {
        fs::write(&template_path, TEMPLATE_CONFIG_CONTENT).await.map_err(|e| {
...
fn merge_config(
    ...
    let main_idx = config_files
        .get_index_of(&config_dir.join(MAIN_CONFIG_FILENAME))
        .expect("Main config must be in config_files");
    let default_idx = config_files
        .get_index_of(&PathBuf::from(DEFAULT_CONFIG_FILENAME))
    ...
    for (key, (value, _file_idx)) in &default_config.values {
        final_values.insert(key.clone(), (value.clone(), default_idx));
    }
        ...
        match final_values.entry(key.clone()) {
            Entry::Occupied(mut entry) => {
                ...
                if preliminary_value.source_file_idx == main_idx {
                    ...
                    entry.insert((new_value.clone(), preliminary_value.source_file_idx));
...
mod tests {
    ...
    fn test_merge_config_example_from_docs() {
        ...
        default_values
            .insert("arcella.log.file".to_string(), (make_toml_value("arcella_default.log"), idx));
        ...
        main_config_values
            .insert("arcella.log.level#redef".to_string(), (make_toml_value("warn"), idx));
        main_config_values
            .insert("arcella.log.file".to_string(), (make_toml_value("arcella_main.log"), idx));
        main_config_values
            .insert("arcella.server.port".to_string(), (make_toml_value("9000"), idx));
        ...
        level_1_values
            .insert("arcella.server.host".to_string(), (make_toml_value("127.0.0.1"), idx)); // This key is not marked as #redef in arcella.toml -> ignored
        level_1_values
            .insert("arcella.server.name".to_string(), (make_toml_value("www.server.net"), idx)); // New key in arcella.server -> ignored
        level_1_values.insert(
            "arcella.custom.message".to_string(),
            (make_toml_value("This is an additional parameter"), idx),
        ); // New key in arcella.custom -> allowed
        ...
        let result =
            merge_config(&default_config, &configs, &config_files, &config_dir, &mut warnings)
                .expect("merge_config should succeed");
        ...
        assert_eq!(
            result.get("arcella.custom.message"),
            Some(&(make_toml_value("This is an additional parameter"), 2))
        ); // From level_1.toml
    ...
    fn test_merge_config_no_redef_prevents_override() {
        ...
        let mut main_config_values: ConfigValues = IndexMap::new();
        main_config_values
            .insert("arcella.server.host".to_string(), (make_toml_value("192.168.1.1"), idx));
        let main_config = TomlFileData {
            includes: vec![],
            values: main_config_values,
        };
        ...
        level_2_values
            .insert("arcella.server.host".to_string(), (make_toml_value("127.0.0.1"), idx));
    ...
    fn test_merge_config_redef_allows_override() {
        ...
        let mut level_1_values: ConfigValues = IndexMap::new();
        level_1_values
            .insert("arcella.log.level#redef".to_string(), (make_toml_value("debug"), idx));
        let level_1_config = TomlFileData {
            includes: vec![],
            values: level_1_values,
        };
    ...
    fn test_merge_config_new_key_in_custom_allowed() {
        ...
        let mut level_1_values: ConfigValues = IndexMap::new();
        level_1_values
            .insert("arcella.custom.new_key".to_string(), (make_toml_value("new_value"), idx));
        let level_1_config = TomlFileData {
            includes: vec![],
            values: level_1_values,
        };
    ...
    fn test_merge_config_new_key_in_server_ignored() {
        ...
        let mut level_1_values: ConfigValues = IndexMap::new();
        level_1_values
            .insert("arcella.server.new_option".to_string(), (make_toml_value("some_value"), 2));
        let level_1_config = TomlFileData {
            includes: vec![],
            values: level_1_values,
        };
    ...
    fn test_extract_nested_subtree() {
        let mut config = ConfigValues::new();
        config.insert(
            "arcella.cache.redis.host".to_string(),
            (TomlValue::String("localhost".into()), 0),
        );
